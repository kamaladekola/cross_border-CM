function build_capacityIC_agent!(mod::Model)
    ...
    # Variables
    cap_import = mod.ext[:variables][:cap_import] =  @variable(mod, [jz=JZ], lower_bound = 0, base_name = "cap_import")
    cap_export = mod.ext[:variables][:cap_export] =  @variable(mod, [jz=JZ], lower_bound = 0, base_name = "cap_export")
    cap_cm = mod.ext[:expressions][:cap_cm] = @expression(mod, [jz=JZ], cap_import[jz] - cap_export[jz])
    ...
    ex_net = mod.ext[:expressions][:ex_net] = @expression(mod, [js=JS,jz=JZ],
    sum(ex_cm[js,t] for t in TCONNECT if t[2] == zone_syms[jz]) -
    sum(ex_cm[js,t] for t in TCONNECT if t[1] == zone_syms[jz]))

    mod.ext[:constraints][:deliverability_import] = @constraint(mod, [js in JS, jz in JZ], ex_net[js,jz] >= cap_import[jz])
    ...
    ## exchanges and flows
        ...
        mod.ext[:constraints][:cap_cm_global_balance] = @constraint(mod, [js in JS],
            sum(g_scarcity[js,jn] - demand[js,jn] + ens_cm[js,jn] for jn in JN) == 0)
    ...
    elseif coupling == "ATC"     # each border is independently constrained by ATC 
        ...
        + ex_net[js,jz] == sum(demand[js,jn] for jn in JN if zone_of_idx[jn] == jz))
