function build_generator_agent!(mod::Model, m::String, zones::Vector{String})
    ...
    mod.ext[:objective] = @objective(mod, Min,
        ...
        + σ_CM * ρ_CM[home_zone]/2 * (sum(cap_cm[jz] for jz in JZ) - cap_bar[home_zone])^2 # ADMM penalty term for CM when all capacity in a zone is remunerated at the same price
